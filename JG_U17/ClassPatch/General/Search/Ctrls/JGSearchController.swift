...
class JGSearchController: JGBaseController {
    ...
    private lazy var historyTableView: UITableView = {
        ...
        tw.delegate = self
        tw.dataSource = self
        tw.register(headerFooterViewType: JGSearchTHeader.self)
        tw.register(cellType: JGBaseTableViewCell.self)
        tw.register(headerFooterViewType: JGSearchTFooter.self)
        return tw
    }()
    ...
    lazy var searchTableView: UITableView = {
        ...
        sw.delegate = self
        sw.dataSource = self
        sw.register(headerFooterViewType: JGSearchTHeader.self)
        sw.register(cellType: JGBaseTableViewCell.self)
        return sw
    }()
    ...
    lazy var resultTableView: UITableView = {
        ...
        rw.delegate = self
        rw.dataSource = self
        rw.register(cellType: JGComicTCell.self)
        return rw
    }()
    ...
    private func loadHistory() {
        ...
        ApiLoadingProvider.request(JGApi.searchHot, model: HotItemsModel.self) { (returnData) in
    ...
    private func searchRelative(_ text: String) {
        if text.count > 0 {
            ...
            currentRequest = ApiProvider.request(JGApi.searchRelative(inputText: text), model: [SearchItemModel].self) { (returnData) in
    ...
    private func searchResult(_ text: String) {
        if text.count > 0 {
            ...
            searchBar.text = text
            ApiLoadingProvider.request(JGApi.searchResult(argCon: 0, q: text), model: SearchResultModel.self) { (returnData) in
...
extension JGSearchController: UITextFieldDelegate {
...
extension JGSearchController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if tableView == historyTableView {
            let head = tableView.dequeueReusableHeaderFooterView(JGSearchTHeader.self)
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        if tableView == historyTableView && section == 1 {
            let foot = tableView.dequeueReusableHeaderFooterView(JGSearchTFooter.self)
            foot?.data = hotItems ?? []
            foot?.didSelectIndexClosure{ [weak self] (index, model) in
                let vc = JGComicController(comicid: model.comic_id)
