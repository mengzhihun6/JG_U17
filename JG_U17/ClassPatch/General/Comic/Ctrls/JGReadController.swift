...
class JGReadController: JGBaseController {
    ...
    private var isLandscapeRight: Bool! {
        didSet {
            JGAppDelegate.changeOrientationTo(landscapeRight: isLandscapeRight)
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        cw.delegate = self
        cw.dataSource = self
        cw.register(cellType: JGReadCCell.self)
    ...
    lazy var bottomBar: JGReadBottomBar = {
        let br = JGReadBottomBar()
    ...
    func loadData(with index: Int, isPreious: Bool, needClear: Bool, finished: ((_ finished: Bool) -> Void)? = nil) {
        ...
        if index <= -1 {
            ...
            ApiLoadingProvider.request(JGApi.chapter(chapter_id: chapterId), model: ChapterModel.self) { (returnData) in
...
extension JGReadController:UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: JGReadCCell.self)
